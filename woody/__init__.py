...
IO_WRAPPER_INSTANCE:"IOWrapper" = None
...
class IOWrapper:
	...
	def write(*args, **kwargs):
		...
		ACTIVE_DEBUG_CONTEXT.simple_log(*args, **kwargs)
...
class Woody:
	...
	for the library.
	"""
	def __init__(self, log_files_path:str="", override_std_io="OverrideType") -> None:
		global WOODY_INSTANCE
		if WOODY_INSTANCE:
		...
		self._handle_io_override(override_std_io)

		try:
			if not os.path.exists(self.log_files_path) and self.log_files_path != "":
				os.mkdir(self.log_files_path)
		except FileExistsError:
			pass
	def _handle_io_override(self, override_std_io:"OverrideType"):
			if override_std_io == OverrideType.THREADING:
				sys.stdout = IOWrapper.write
				sys.stderr = IOWrapper.write
